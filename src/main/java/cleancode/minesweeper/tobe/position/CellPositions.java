...
public class CellPositions {
    ...
    public List<CellPosition> extractRandomPositions(int count) { //얘 입장에선 landmineCount인지 모르므로 count
        List<CellPosition> cellPositions = new ArrayList<>(positions);

        Collections.shuffle(cellPositions); //shuffle 하면 내부 요소들 순서 바뀌므로 필드에 영향가지 않도록 새로운 컬렉션 생성하여 연산
        return cellPositions.subList(0, count);
    }

    public List<CellPosition> subtract(List<CellPosition> positionListToSubtract) { //얘 입장에선 landminePositions인지 모르므로 positions
        //뺄셈 연산을 해야하므로 새로운 리스트 생성하기
        List<CellPosition> cellPositions = new ArrayList<>(positions);
